...
public class CapsuleHand : IHandModel {
  ...
  void OnEnable ()
    {
        EventManager.StartListening("updateHandVisibility", UpdateVisibility);
    }
  void OnDisable()
    {
        EventManager.StopListening("updateHandVisibility", UpdateVisibility);
    }

  public void UpdateVisibility()
    {
        // Called when toggling visibility from script not Editor
  ...
  private void createSpheres() {
    ...
    for (int i = 0; i < fingers.Count; i++) {
      ...
      for (int j = 0; j < 4; j++) {
        int key = getFingerJointIndex((int)finger.Type, j);
        _jointSpheres[key] = createSphere("Joint", SPHERE_RADIUS, false, true);
      }
    }

    mockThumbJointSphere = createSphere("MockJoint", SPHERE_RADIUS, false, true);
    palmPositionSphere = createSphere("PalmPosition", PALM_RADIUS, false, true);
    wristPositionSphere = createSphere("WristPosition", SPHERE_RADIUS, false, true);
  ...
  private void createCapsules() {
    ...
    for (int i = 0; i < 5; i++) {
      for (int j = 0; j < 3; j++) {
        ...
        createCapsule("Finger Joint", sphereA, sphereB, false, true);
      }
    }
    ...
    for (int i = 0; i < 4; i++) {
      ...
      createCapsule("Hand Joints", sphereA, sphereB, false, true);
    }

    //Create the rest of the hand
    Transform thumbBase = _jointSpheres[THUMB_BASE_INDEX];
    Transform pinkyBase = _jointSpheres[PINKY_BASE_INDEX];
    createCapsule("Hand Bottom", thumbBase, mockThumbJointSphere, false, true);
    createCapsule("Hand Side", pinkyBase, mockThumbJointSphere, false, true);
  ...
  private Transform createSphere(string name, float radius, bool isPartOfArm = false, bool isPartOfHand = false) {
  ...
  private void createCapsule(string name, Transform jointA, Transform jointB, bool isPartOfArm = false, bool isPartOfHand = false) {
    ...
    if (isPartOfHand) { 
      _handRenderers.Add(capsule.GetComponent<Renderer>());
    }
  }
}
